...
SELECT
    ...
    t1.staff_id,
    t1.active,
    CONCAT(t1.first_name, ' ', t1.last_name) AS staff_name
FROM staff AS t1
LEFT JOIN
(
	SELECT 
...
-- # of rents for each store
SELECT
	YEAR(t1.rental_date) AS year,
	MONTH(t1.rental_date) AS month,
	t2.store_id,
	COUNT(*) AS n_rental
FROM rental AS t1
LEFT JOIN
